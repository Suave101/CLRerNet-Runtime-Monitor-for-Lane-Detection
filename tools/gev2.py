...
def noisy_test_step(self, data, sigma=0.0):
    """
    Injected noise logic with DDP awareness.
    """
    ...
    if get_rank() == 0:
        ...
        if model_base.vis_count < 5:
            vis_dir = f"verification_sigma_{sigma}"
            os.makedirs(vis_dir, exist_ok=True)
            img_to_save = data['inputs'][0].detach().cpu()
            save_path = os.path.join(vis_dir, f"sample_{model_base.vis_count}.png")
